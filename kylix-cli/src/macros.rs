...
macro_rules! kem_keygen {
    ($algo:ty) => {{
        let (dk, ek) = <$algo>::keygen(&mut ::rand::rng())
            .map_err(|e| ::anyhow::anyhow!("Key generation failed: {:?}", e))?;
        (
            ek.as_bytes().to_vec(),
            ::zeroize::Zeroizing::new(dk.as_bytes().to_vec()),
        )
    }};
}
...
macro_rules! dsa_keygen {
    ($algo:ty) => {{
        let (sk, pk) = <$algo>::keygen(&mut ::rand::rng())
            .map_err(|e| ::anyhow::anyhow!("Key generation failed: {:?}", e))?;
        (
            pk.as_bytes().to_vec(),
            ::zeroize::Zeroizing::new(sk.as_bytes().to_vec()),
        )
    }};
}
...
macro_rules! kem_encaps {
    ($crate_:ident :: $submod:ident, $algo:ty, $pk_bytes:expr) => {{
        ...
        let (ct, ss) = <$algo>::encaps(&ek, &mut ::rand::rng())
...
macro_rules! dsa_verify {
    ($crate_:ident :: $submod:ident, $algo:ty, $pk_bytes:expr, $sig_bytes:expr, $message:expr) => {{
        let pk = $crate_::$submod::VerificationKey::from_bytes(&$pk_bytes)
            .map_err(|e| ::anyhow::anyhow!("Invalid verification key: {:?}", e))?;
